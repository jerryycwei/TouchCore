...
public final class MathUtils {
    ...
    public static boolean pointIsInRectangle(Vector3D point, RamRectangleComponent rectangle, int margin) {
        ...
        Vector3D horizontalShift = new Vector3D(Math.min(margin, rectangle.getWidth()), 0);
        Vector3D verticalShift = new Vector3D(0, Math.min(margin, rectangle.getHeight()));

        return rectangle.containsPointGlobal(point.getAdded(horizontalShift))
                || rectangle.containsPointGlobal(point.getSubtracted(horizontalShift))
                || rectangle.containsPointGlobal(point.getAdded(verticalShift))
                || rectangle.containsPointGlobal(point.getSubtracted(verticalShift));
    }
}
