...
public class DisplayAspectScene extends RamAbstractScene<IDisplaySceneHandler>
    ...
    public DisplayAspectScene(RamApp app, Aspect aspect, String sceneName) {
        ...
        /* Build instantiations container (container) */

        instantiationsPanel = new InstantiationsPanel(this);
        containerLayer.addChild(instantiationsPanel);
        instantiationsPanel.setPositionGlobal(new Vector3D(-CONTAINER_HIDDEN_PART_FROM_LEFT,
                RamApp.getApplication().getSize().height - instantiationsPanel.getHeight(), 0));
        instantiationsPanel.setHandler(HandlerFactory.INSTANCE.getInstantiationsPanelHandler());
        instantiationsPanel.setBottomStick(true);

        InstantiationsContainerView aspectInstContainerView = new InstantiationsContainerView(this, false,
                Strings.LABEL_CONTAINER_MODEL_EXTENDS);
        aspectInstContainerView.setHandler(HandlerFactory.INSTANCE.getAspectExtensionsContainerViewHandler());
        InstantiationsContainerView modelReuseContainerView =
                new InstantiationsContainerView(this, true, Strings.LABEL_CONTAINER_MODEL_REUSE);
        modelReuseContainerView.setHandler(HandlerFactory.INSTANCE.getModelReuseContainerHandler());

        instantiationsPanel.addInstantiationsContainerView(aspectInstContainerView);
        instantiationsPanel.addInstantiationsContainerView(modelReuseContainerView);
    ...
    public void actionPerformed(ActionEvent event) {
        if (handler != null) {
            ...
            } else if (ACTION_WEAVE_ALL.equals(actionCommand)) {
    ...
    public boolean destroy() {
        ...
        AdapterFactoryRegistry.INSTANCE.disposeAdapterFactoryFor(aspect);
    ...
    public void showMessageView(AbstractMessageView messageView) {
        ...
        if (view == null) {
            ...
            // Temporarily add the view to be able to set the handler.
            viewContainer.addChild(view);
            view.setHandler(ca.mcgill.sel.ram.ui.views.message.handler.HandlerFactoryMessageView.INSTANCE
