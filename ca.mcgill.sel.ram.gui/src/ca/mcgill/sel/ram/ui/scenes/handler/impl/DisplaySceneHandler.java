 ...
 * @author mschoettle
 */
public class DisplaySceneHandler extends DefaultRamSceneHandler implements IDisplaySceneHandler {
    ...
    public void weaveAll(DisplayAspectScene scene) {
        // TODO: mschoettle: Add a more elaborate check if there are no instantiations left. Check if there are unwoven
    ...
    private void doSwitchToConcern(DisplayAspectScene scene) {
        RAMModelUtil.unloadExternalResources(scene.getAspect());

        ResourceManager.unloadResource(scene.getAspect().eResource());
        scene.setTransition(new SlideTransition(RamApp.getApplication(), 700, false));
     ...
     * @param scene the current aspect scene
     */
    private static void checkForModelRemoval(DisplayAspectScene scene) {
