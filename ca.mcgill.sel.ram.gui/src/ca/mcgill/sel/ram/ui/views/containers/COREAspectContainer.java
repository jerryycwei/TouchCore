 ...
 * @author jerrywei
 */
public class COREAspectContainer extends RamPanelListComponent<COREModel> {

    private static final int DEFAULT_MAX_ELEMENT = 8;
    private static final int DEFAULT_ARC_RADIUS = 5;

    private COREModel selected;
     ...
     */
    public RamListComponent<COREModel> getSelector() {
        return list;
    }
     ...
     * @param scene - The current scene
     */
    private void select(COREModel element, DisplayConcernEditScene scene) {
        ...
        if (firstSelection) {
            ...
            scene.updateColorsFromSelection();
        }
        RamRectangleComponent component = list.getElementMap().get(selected);
     ...
     * @author CCamillieri
     */
    private class AspectNamer implements Namer<COREModel> {
        @Override
        public RamRectangleComponent getDisplayComponent(final COREModel element) {
                    ...
                    deleteConfirmPopup.setListener(new ConfirmPopup.SelectionListener() {
                        ...
                        public void optionSelected(int selectedOption) {
                            if (selectedOption == ConfirmPopup.YES_OPTION) {
                                currentScene.getHandler().deleteRealisationModel(currentScene, element);
                                if (selected == element) {
                                    selected = null;
                                    currentScene.switchToEditMode();
                                } else if (selected != null) {
                                    currentScene.updateColorsFromSelection();
                                }
                                return;
                            }
                        }
     ...
     * @author Nishanth
     */
    private class Listener implements RamListListener<COREModel> {
         ...
         */
        @Override
        public void elementSelected(RamListComponent<COREModel> selectorPassed, COREModel element) {
        ...
        @Override
        public void elementDoubleClicked(RamListComponent<COREModel> selectorPassed, COREModel element) {
            currentScene.setTransition(new SlideTransition(RamApp.getApplication(), 700, true));
            if (element instanceof Aspect) {
                RamApp.getApplication().loadAspect((Aspect) element);
            } else if (element instanceof UseCaseMap) {
                RamApp.getApplication().loadUCM((UseCaseMap) element);
            }
        }
    ...
    public final void refreshContent() {
        Set<COREModel> proxies = new HashSet<COREModel>();
        for (COREModel element : list.getElementMap().keySet()) {
            if (element.eIsProxy()) {
                proxies.add(element);
            }
        }
        // Remove the elements from the list and add them again.s
        for (COREModel element : proxies) {
            COREModel resolvedAspect = (COREModel) EcoreUtil.resolve(element, currentScene.getConcern());
